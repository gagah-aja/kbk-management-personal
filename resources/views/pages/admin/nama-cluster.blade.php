                ...
                <form action="{{ route('admin.nama-cluster.store') }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="namaCluster" name="nama_cluster" placeholder="Masukkan nama cluster" required>
                        </div>
                    </div>
            ...
            <tbody>
                @foreach($namaClusters->sortBy('id') as $cluster)
                    <tr class="text-center">
                        ...
                        <td>
                            {{-- Tombol Edit --}}
                            <button type="button" class="btn btn-sm btn-warning text-white" data-bs-toggle="modal" data-bs-target="#editModal{{ $cluster->id }}">
                                Edit
                            </button>

                            {{-- Tombol Hapus --}}
                            <form action="{{ route('admin.nama-cluster.destroy', $cluster->id) }}" method="POST" class="d-inline form-hapus">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-danger btn-hapus" data-nama="{{ $cluster->nama_cluster }}">Hapus</button>
                            </form>
                        </td>
                    </tr>

                    {{-- Modal Edit --}}
                    <div class="modal fade" id="editModal{{ $cluster->id }}" tabindex="-1" aria-labelledby="editModalLabel{{ $cluster->id }}" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                ...
                                <form action="{{ route('admin.nama-cluster.update', $cluster->id) }}" method="POST">
                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" id="namaCluster{{ $cluster->id }}" name="nama_cluster" value="{{ $cluster->nama_cluster }}" required>
                                        </div>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
