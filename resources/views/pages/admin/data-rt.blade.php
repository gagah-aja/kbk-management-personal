                    ...
                    @forelse($dataRT as $index => $rt)
                        <tr>
                            ...
                            <td>
                                ...
                                {{-- Tombol Hapus --}}
                                <form action="{{ route('admin.data-rt.destroy', $rt->id) }}" method="POST" class="d-inline"
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select name="id_warga" class="form-select" required>
                                                    ...
                                                    @foreach($warga as $w)
                                                        @php
                                                            $sudahJadiRT = in_array($w->id, $idWargaSudahRT ?? []);
                                                        @endphp
                                                        <option value="{{ $w->id }}"
                                                            {{ $rt->id_warga == $w->id ? 'selected' : ($sudahJadiRT ? 'disabled' : '') }}>
                                                            {{ $w->nama_lengkap }}
                                                            @if($sudahJadiRT && $rt->id_warga != $w->id)
                                                                — Sudah jadi Ketua RT
                                                            @endif
                                                        </option>
                                                    @endforeach
                        ...
                        <select name="id_warga" class="form-select" required>
                            ...
                            @foreach($warga as $w)
                                @php
                                    $sudahJadiRT = in_array($w->id, $idWargaSudahRT ?? []);
                                @endphp
                                <option value="{{ $w->id }}" {{ $sudahJadiRT ? 'disabled' : '' }}>
                                    {{ $w->nama_lengkap }}
                                    @if($sudahJadiRT)
                                        — Sudah jadi Ketua RT
                                    @endif
